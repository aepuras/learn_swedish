                ...
                const index = user.words.findIndex(word => {
                    return (
                        JSON.stringify(word.english) ===
                            JSON.stringify(oldWord.english) &&
                        JSON.stringify(word.swedish) ===
                            JSON.stringify(oldWord.swedish) &&
                        (!oldWord.learned ||
                            word.learned === oldWord.learned) &&
...
function saveUserData(userId, res, updateUserCallback) {
    User.findById(userId, (userErr, user) => {
        if (userErr || !user) {
            console.log(userErr);
            res.status(401).json({
